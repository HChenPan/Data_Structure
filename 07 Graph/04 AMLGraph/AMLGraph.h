//
//文件夹: 07 Graph\04 AMLGraph
//文件名: AMLGraph
//内　容：无向图（邻接多重表）相关操作列表
//Author Huangcp
//Date 2019/9/13 上午 02:40
//Version 1.0
//
...
void DFS_AML(AMLGraph G, int v) {
    ...
    for (w = FirstAdjVex_AML(G, G.adjmulist[v].data); w; w = NextAdjVex_AML(G, G.adjmulist[v].data,
                                                                            G.adjmulist[w].data)) {
        if (!visited[w])
            DFS_AML(G, w);
    }
    ...
    for (v = 1; v <= G.vexnum; v++) {
        if (!visited[v]) {
            ...
            while (!QueueEmpty_L(Q)) {
                DeQueue_L(&Q, &e);
                for (w = FirstAdjVex_AML(G, G.adjmulist[e].data); w; w = NextAdjVex_AML(G, G.adjmulist[e].data,
                                                                                        G.adjmulist[w].data)) {
                    if (!visited[w]) {
                        visited[w] = TRUE;
