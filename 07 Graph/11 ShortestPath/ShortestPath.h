...
#ifndef DATA_STRUCTURE_SHORTESTPATH_H
#define DATA_STRUCTURE_SHORTESTPATH_H
...
**/
void ShortestPath_FLOYD(MGraph G2, PathMatrix P2[][MAX_VERTEX_NUM + 1][MAX_VERTEX_NUM + 1],
                        ShortPathTable D2[][MAX_VERTEX_NUM + 1]);

/**
(05)输出用Floyd算法求出的有向图路径。 
**/
void OutputPath_FLOYD(MGraph G2, PathMatrix P2[][MAX_VERTEX_NUM + 1][MAX_VERTEX_NUM + 1],
                      ShortPathTable D2[][MAX_VERTEX_NUM + 1]);
...
void ShortestPath_DIJ(MGraph G1, int v0, PathMatrix P1[][MAX_VERTEX_NUM + 1], ShortPathTable D1[]) {
    ...
    for (i = 2; i <= G1.vexnum; i++)                    //其余G1.vexnum-1个顶点
    {
        ...
        {
            if (!final[w] && min < INFINITY && G1.arcs[v][w].adj < INFINITY &&
                (min + G1.arcs[v][w].adj < D1[w]))//修改D1[w]和P1[w],w∈V-S
            {
...
void ShortestPath_FLOYD(MGraph G2, PathMatrix P2[][MAX_VERTEX_NUM + 1][MAX_VERTEX_NUM + 1],
    ...
    for (u = 1; u <= G2.vexnum; u++) {
        for (v = 1; v <= G2.vexnum; v++) {
            for (w = 1; w <= G2.vexnum; w++) {
                if (v != w && D2[v][u] < INFINITY && D2[u][w] < INFINITY &&
                    D2[v][u] + D2[u][w] < D2[v][w]) //从v经u到w的一条路径更短
                {
...
void Floyd(MGraph G2, PathMatrix path2[][MAX_VERTEX_NUM + 1], ShortPathTable dist2[][MAX_VERTEX_NUM + 1]) {
    ...
    for (k = 1; k <= G2.vexnum; k++)                            //以k为中间点检测各对顶点间距离
    {
        for (i = 1; i <= G2.vexnum; i++) {
            for (j = 1; j <= G2.vexnum; j++) {
                if (i != j && dist2[i][k] < INFINITY && dist2[k][j] < INFINITY &&
                    dist2[i][k] + dist2[k][j] < dist2[i][j]) {
...
#endif //DATA_STRUCTURE_SHORTESTPATH_H
